...
export var gap = UNIT_SIZE * 5  # Gap horizontal entre chaque ilot
export var gap_offset = 7  # Gap vertical = gap_offset*UNIT_SIZE
export(int) var number_map = 2  # Nombre d'ilot sans trou genere sur la map
export(int) var number_stage = 1  # Nombre d'ilot sans trou genere sur la map
		...
		for j in range(number_stage):
			...
			next_pos = add_sol(
				(
					...
					+ i * gap_offset * UNIT_SIZE * Vector2(0, 1)
				),
				is_final_map
			)
