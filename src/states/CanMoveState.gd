...
#	out_velocity.x = player.max_speed.x * player.direction.x
	out_velocity.x = lerp(out_velocity.x, player.max_speed.x * input_speed, 0.8) #_get_h_weight(player, input_speed))
#	out_velocity.y += player.gravity * delta
#	TODO IS_JUMPING
#	if player.direction.y == -1.0:
#		out_velocity.y = player.max_speed.y * player.direction.y
...
#		return 0.2  # 0.2
		...
		elif input_speed == sign(player.direction.x) and abs(player.direction.x) > player.max_speed:  # max_jump_velocity:  # move_speed:
			return 0.1
		else:
			return 0.2
